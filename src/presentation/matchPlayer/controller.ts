...
import { CustomError, PaginationDTO, CreateMatchPlayerDto } from "../../domain";
...
export class MatchPlayerController {
  ...
  public createMatchPlayer = async (req: Request, res: Response) => {
    ...
    const [error, createMatchPlayerDto] = CreateMatchPlayerDto.create(req.body);

    if (error) {
      return res.status(400).json({ message: error });
    }

    this.matchPlayerService.createMatchPlayer(createMatchPlayerDto!)
